...
import { useInstituteClasses, useCreateInstituteClass, useUpdateInstituteClass, useDeleteInstituteClass } from '../hooks/useClasses';

const AdminInstituteClasses = () => {
  const { data: instituteClassesResponse, isLoading: loadingClasses } = useInstituteClasses();
  const instituteClasses = instituteClassesResponse?.data || [];

  const createClassMutation = useCreateInstituteClass();
  const updateClassMutation = useUpdateInstituteClass();
  const deleteClassMutation = useDeleteInstituteClass();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error saving institute class:', error);
      alert('Error saving institute class');
    }
  };
            ...
            <div key={classItem.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all">
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${classItem.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                    {classItem.status}
                  </span>
                </div>
